...
export default function PastAppointments({name, time, date, service, instagram, status, bookingStatus}) {
    return (
        <div className='flex flex-row justify-start items-start bg-white/10  border border-white/20 rounded-lg p-4 gap-4'>
            ...
            <div className='flex flex-row items-center  rounded-lg p-2 gap-4 ml-auto self-center'>
               ...
               {status==="Verified" ? <button className={UnavailableVerifiedTimeslotsStyle}>Unverify</button> : <button className={UnavailableUnverifiedTimeslotsStyle}>Verify</button>}
            </div>
        </div>
    )
}